...
contract TestMathLib {
    ...
    function testSubtract() public {
        Assert.equal(
            MathLib.subtract(uint256(1), uint256(1)),
            0,
    ...
    function testcalculateCollateralToReturnLong() public {
        uint256 priceFloor = 250;
        uint256 priceCap = 350;
        uint256 qtyMultiplier = 100;
        uint256 price = 275;
        uint256 longQty = 2;
        uint256 neededCollateralForLongPos = MathLib.calculateCollateralToReturn(
            priceFloor,
            priceCap,
            qtyMultiplier,
            longQty,
    ...
    function testcalculateCollateralToReturnShort() public {
        ...
        uint256 neededCollateralForShortPos = MathLib.calculateCollateralToReturn(
            priceFloor,
            priceCap,
            qtyMultiplier,
            0,
    ...
    function testCalculateTotalCollateralSingleUnit() public {
        uint256 priceFloor = 10;
        uint256 priceCap = 20;
        uint256 multiplier = 1;
        uint256 expectedTotalCollateral = 10;
        uint256 actualTotalCollateral = MathLib.calculateTotalCollateral(
            priceFloor,
            priceCap,
            multiplier
        );
    ...
    function testCalculateTotalCollateralMultipleUnit() public {
        ...
        uint256 expectedTotalCollateral = 20;
        uint256 actualTotalCollateral = MathLib.calculateTotalCollateral(
            priceFloor,
            priceCap,
            multiplier
    ...
    function testCalculateFeePerUnit() public {
        uint256 priceFloor = 1000;
        uint256 priceCap = 2000;
        uint256 multiplier = 1000;
        uint256 feeAmountInBasis = 100; // 1 percent fee.
        uint256 fee = MathLib.calculateFeePerUnit(
            priceFloor,
            priceCap,
            multiplier,
            feeAmountInBasis
        );
        uint256 expectedFeeAmount = 15000; // midpoint * multiplier * 1% (100 basis points)
        Assert.equal(
            fee,
            expectedFeeAmount,
