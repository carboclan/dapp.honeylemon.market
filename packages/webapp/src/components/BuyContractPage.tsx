  ...
  useEffect(() => {
    const getQuoteForBudget = async () => {
      ...
      try {
        ...
        const newIsLiquid = !!(
          Number(result?.remainingTakerFillAmount?.toString() || -1) === 0
        );
        const newOrderValue =
          Number(
            result?.totalTakerFillAmount?.shiftedBy(-PAYMENT_TOKEN_DECIMALS).toString()
          ) || 0;
        const collateralRequiredForPosition = await honeylemonService.calculateRequiredCollateral(
          new BigNumber(result.totalMakerFillAmount)
        );
        const newExpectedAccrual = Number(
          new BigNumber(collateralRequiredForPosition)
            .shiftedBy(-COLLATERAL_TOKEN_DECIMALS)
            .dividedBy(CONTRACT_COLLATERAL_RATIO)
            .toString()
        );
        const { currentBTCSpotPrice } = marketData;
        const discountValue = !isLiquid
          ? 0
          : ((currentBTCSpotPrice - newOrderValue / newExpectedAccrual) /
              currentBTCSpotPrice) *
            100;
    ...
    const getQuoteForSize = async () => {
      ...
      try {
        const result = await honeylemonService.getQuoteForSize(
          new BigNumber(orderQuantity)
        );
        const newIsLiquid = !!(
          Number(result?.remainingMakerFillAmount?.toString() || -1) === 0
        );
        const newOrderValue =
          Number(
            result?.totalTakerFillAmount?.shiftedBy(-PAYMENT_TOKEN_DECIMALS).toString()
        ...
        const newExpectedAccrual = Number(
          new BigNumber(
            await honeylemonService.calculateRequiredCollateral(
              new BigNumber(orderQuantity)
            )
          )
            .shiftedBy(-COLLATERAL_TOKEN_DECIMALS)
      ...
      <Grid container alignItems="center" justify="flex-start" spacing={2}>
        ...
        <Grid item xs={12} container>
          <Grid item xs={12} style={{ paddingLeft: 0, paddingRight: 0 }}>
            <Paper
              className={clsx(classes.orderSummary, {
                ...
                <TableBody>
                  ...
                  <TableRow>
                    ...
                    <TableCell align="right" className={classes.orderSummaryEstimate}>
                      ...
                      })}% ${discountOnSpotPrice < 0 ? "Premium" : "Discount"}`}
                    </TableCell>
                  </TableRow>
                  {!showOrderDetails ? (
                    <TableRow>
